#!/usr/bin/env python3
"""
Resolution analysis for Nek5000 full spherical shell simulations.
...
structured grids, then computes:
  ...
  2. Azimuthal Fourier spectra of velocity and temperature in the equatorial
     plane (using spherical velocity components)
  3. Chebyshev radial spectra of RMS fields
  4. Equatorial cross-section contours of fluctuating fields

Must be run with MPI:
    mpirun -np N python sphere_resolution.py [field_file]
...
def build_radial_grid(ri, ro, nr, lx, radial_dist='uniform'):
    """Build GLL radial grid: nr elements with lx GLL points each.

    Supports uniform or Chebyshev element spacing.
...
parser = argparse.ArgumentParser(
    description='Resolution analysis for Nek5000 full spherical shell simulations')
...
parser.add_argument('--nphi-shell', type=int, default=128,
...
parser.add_argument('--Ek', type=float, default=1e-3,
                    help='Ekman number (for BL annotation)')
parser.add_argument('--radial-dist', default='uniform',
                    choices=['uniform', 'chebyshev'],
                    help='Radial element distribution (default: uniform)')
...
phi_shell_1d = np.linspace(eps, 2 * np.pi - eps, nphi_shell)
# Equatorial probes: uniform grid on [0, 2*pi) for clean FFT
phi_eq_1d = np.linspace(0, 2 * np.pi, nphi_eq, endpoint=False)
...
if rank == 0:
    ...
    # Shell averages with sin(theta) weighting for proper spherical integration
    TH_w, _ = np.meshgrid(theta_1d, phi_shell_1d, indexing='ij')
    sin_w = np.sin(TH_w.ravel())  # length n_shell
    sin_w = sin_w / np.sum(sin_w)  # normalize

    ur_mean = np.sum(ur_sh * sin_w[None, :], axis=1)
    ut_mean = np.sum(ut_sh * sin_w[None, :], axis=1)
    up_mean = np.sum(up_sh * sin_w[None, :], axis=1)
    T_mean = np.sum(T_sh * sin_w[None, :], axis=1)

    # Shell RMS (sin-weighted)
    ur_rms = np.sqrt(np.sum((ur_sh - ur_mean[:, None])**2 * sin_w[None, :], axis=1))
    ut_rms = np.sqrt(np.sum((ut_sh - ut_mean[:, None])**2 * sin_w[None, :], axis=1))
    up_rms = np.sqrt(np.sum((up_sh - up_mean[:, None])**2 * sin_w[None, :], axis=1))
    T_rms = np.sqrt(np.sum((T_sh - T_mean[:, None])**2 * sin_w[None, :], axis=1))
    ...
    # Fourier spectra: mode k = azimuthal wavenumber m (full sphere)
    ...
    # Save spectra CSV
    spec_out = np.column_stack((m_modes, E_kin_total, E_T_total))
    np.savetxt('equatorial_spectra_total.csv', spec_out, delimiter=',',
               header='m, E_kin, E_T')
    print("Saved equatorial_spectra_total.csv")

    # Per-radius spectra at selected interior radii
    trim = max(1, nr_pts // 10)
    ...
    fig, ax = plt.subplots(figsize=(5, 4), tight_layout=True)
    mask_m = m_modes > 0
    ax.loglog(m_modes[mask_m], E_kin_total[mask_m], 'k-', linewidth=1.5,
              label='integrated')
    colors = plt.cm.viridis(np.linspace(0, 1, len(idx_sel)))
    for ci, i in enumerate(idx_sel):
        ax.loglog(m_modes[mask_m], E_kin[i, mask_m], color=colors[ci],
    ...
    fig, ax = plt.subplots(figsize=(5, 4), tight_layout=True)
    ax.loglog(m_modes[mask_m], E_T_total[mask_m], 'k-', linewidth=1.5,
              label='integrated')
    for ci, i in enumerate(idx_sel):
        ax.loglog(m_modes[mask_m], E_T[i, mask_m], color=colors[ci],
                  alpha=0.7, label=f'r={r_1d[i]:.3f}')
    ax.set_xlabel('m')
